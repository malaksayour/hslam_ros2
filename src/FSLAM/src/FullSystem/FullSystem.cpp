...
Vec5 FullSystem::trackNewCoarse(FrameHessian* fh, bool writePose)
{
	...
	else
	{
		...
		SE3 lastF_2_slast;
		{
			...
			SE3 TESTVAL1=slast->getPose();
			SE3 TESTVAL2=sprelast->getPose().inverse();
			SE3 testval3=TESTVAL2 * TESTVAL1;
			int x=0;
			SE3 slast_2_sprelast = TESTVAL2 * TESTVAL1;
			SE3 MALAK1=slast->getPose().inverse();

			FrameShell* MALAK3=lastF->shell;
			SE3 MALAK2=MALAK3->getPose();
			lastF_2_slast = MALAK1 * MALAK2; 
...
{ 	/*The addActiveFrame function in the FullSystem class incorporates a new image frame into the system for processing and state estimation. It performs the following key steps:

	Initialization: Ensures system readiness and acquires a lock for thread safety.
	Frame Creation: Constructs frame instances and adds them to the system's history.
	Image Processing: Extracts image information and processes it for further analysis.
	Initialization or Tracking: Depending on system status, initializes or continues system setup.
	Tracking and Optimization: Executes tracking and pose optimization algorithms to update camera pose estimates.
	Keyframe Handling: Determines keyframe status and updates global motion models as needed.
	Output and Cleanup: Publishes results and releases locks for thread access.
	This function plays a central role in integrating new frames, maintaining system state, and ensuring accurate localization and mapping within the application.*/
	//Early Exit Check: 
    if(isLost) {std::cout<< "lost"<< std::endl; return;}

	boost::unique_lock<boost::mutex> lock(trackMutex);

	// =========================== add into allFrameHistory =========================
	//Frame Creation and Initialization:
	//While FrameShell manages the high-level information and state of a frame, FrameHessian seems to handle the low-level computations and processing tasks associated with the frame.
	//For example, FrameShell might be responsible for managing the pose of the frame and tracking-related information, while FrameHessian might handle computations related to frame energy, gradient calculations, and optimization.
...
// void FullSystem::addLoopClosure(std::shared_ptr<LoopCloser> loopObject){
// 	loopCloser=loopObject;
// }
...
{	
	...
	printf("Looping thread has been joined\n"); //debugNA
